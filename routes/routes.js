...
routes.get("/", async (req, res) => {
  ...
  const banners = (bannersData || []).filter(b => b.activo).map(b => {
    ...
    const videoEmbed = (() => {
      ...
      if (ytMatch) {
        const id = ytMatch[1];
        return `https://www.youtube.com/embed/${id}?autoplay=1&mute=1&loop=1&playsinline=1&controls=0&modestbranding=1&rel=0&iv_load_policy=3&cc_load_policy=0&disablekb=1&fs=0&playlist=${id}`;
      }
      const vimeoMatch = url.match(/vimeo\.com\/(\d+)/);
      if (vimeoMatch) {
        const id = vimeoMatch[1];
        return `https://player.vimeo.com/video/${id}?autoplay=1&muted=1&loop=1&controls=0&title=0&byline=0&portrait=0`;
      }
      return null;
    })();
    return {
