...
function checkInputFile() {
	...
	if (formSendFile != null) {
		formSendFile.addEventListener('submit', (e) => {
			...
			if (document.querySelector("#img-file").files.length > 0) {
				img = document.querySelector("#img-file").files[0].name;
				//img
				if (!allowedImgExtensions.exec(img)) {
					e.preventDefault();
					alert('Archivo de imágen inválido')
					window.location.reload();
				}
			}
		});
...
function addNavSpecial() {
	const menus = document.getElementsByClassName('nav-special')
	const menusArray = [...menus];
	console.log(menusArray)
	menusArray.forEach((m, i) => m.addEventListener('click', (x => {
		m.classList.add('add-color')
	})))
