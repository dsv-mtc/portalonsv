...
function initForm(): FormData {
  return { value: "", icon: "", estaActivo: true };
}
...
export function DatosAbiertosCategorias() {
  ...
  const load = () => { setPage(1); apiGet<Cat[]>("/datos-abiertos-categorias").then(setItems).catch(() => {}); };
  useEffect(() => { load(); }, []);
  ...
  const openEdit = (item: Cat) => {
    setForm({ value: item.value, icon: item.icon || "", estaActivo: item.estaActivo });
  ...
  const handleFile = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    const r: any = await apiUpload("/datos-abiertos-categorias/upload", fd);
    if (r.success && r.url) {
      setForm(p => ({ ...p, icon: r.url }));
    } else {
  ...
  const handleSubmit = async () => {
    ...
    if (editingId !== null) {
      await apiPut(`/datos-abiertos-categorias/${editingId}`, form);
      setMsg("Categoría actualizada");
    } else {
      await apiPost("/datos-abiertos-categorias", form);
  ...
  const handleDelete = async () => {
    ...
    await apiDelete(`/datos-abiertos-categorias/${id}`);
    setMsg("Eliminada");
    load();
  };
  ...
  return (
    <>
      <PageHeader title="Categorías — Datos Abiertos" eyebrow="Clasificación" actions={
        ...
        <table style={{ width: "100%", borderCollapse: "collapse", fontSize: "13px" }}>
          <thead>
            <tr style={{ borderBottom: "2px solid var(--brand-line)" }}>
              ...
              <th style={{ width: 80, textAlign: "left", padding: "10px 8px", fontWeight: 700, color: "var(--brand-navy)", fontSize: "12px", textTransform: "uppercase", fontFamily: "var(--font-cond)", letterSpacing: "0.06em" }}>Ícono</th>
            ...
            {paginatedItems.map(item => (
              <tr key={item.id} style={{ borderBottom: "1px solid var(--brand-line)" }}>
                ...
                <td style={{ padding: "10px 8px" }}>
                  {item.icon ? (
                    <img className="region-img" src={encodeURI(item.icon)} alt=""
            ...
            <div className="space-y-4">
              ...
              <label className="block">
                ...
                <div className="flex gap-2 items-center mt-1">
                  ...
                  {form.icon && (
                    <img src={encodeURI(form.icon)} className="w-11 h-11 rounded object-contain border" alt="" style={{ borderColor: "var(--brand-line)", background: "#f8fafc" }} />
                  )}
                </div>
              </label>

              <div className="flex items-center gap-2">
                <input type="checkbox" id="catActive" checked={form.estaActivo} onChange={e => setForm(p => ({ ...p, estaActivo: e.target.checked }))}
                  className="w-4 h-4 rounded border-2 accent-[color:var(--brand-navy)]" />
                <label htmlFor="catActive" className="text-[13px] font-semibold" style={{ color: "var(--brand-navy)" }}>¿Está activo?</label>
              </div>
            </div>
