...
export function Comunicaciones() {
  ...
  const [newItem, setNewItem] = useState({ title: "", idTipoEvento: 0, organizedBy: "", place: "", startDay: "", startTime: "", isActive: true });
  ...
  const handleCreate = async () => {
    ...
    setNewItem({ title: "", idTipoEvento: 0, organizedBy: "", place: "", startDay: "", startTime: "", isActive: true });
    load();
  };
    ...
    <>
      ...
      <Panel title="Nuevo evento">
        <div className="grid sm:grid-cols-3 gap-3">
          ...
          <label className="block">
            <span className="text-[10px] uppercase tracking-[0.08em] font-bold font-[family-name:var(--font-cond)]" style={{ color: "var(--brand-navy)" }}>Hora inicio</span>
            <input type="time" value={newItem.startTime} onChange={e => setNewItem(p => ({ ...p, startTime: e.target.value }))} className="mt-1 w-full h-10 rounded-lg border-2 px-3 text-[13px] outline-none" style={{ borderColor: "var(--brand-line)" }} />
          </label>
        </div>
