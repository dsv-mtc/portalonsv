...
/**
 * @description Capa de transiciones suaves (sin dependencias).
 *  - Reveal on scroll (IntersectionObserver) sobre elementos con .fx-hide.
 *  - Sin fade-in de body: la navegación entre páginas NO produce flash blanco.
 *  - Seguro: sólo oculta cuando <html> tiene .fx-ready + .fx-activated (los
 *    elementos visibles ya fueron marcados .fx-in antes de activar el gating).
 *  - Respeta prefers-reduced-motion (el CSS neutraliza todo).
 */
function pageFx() {
  var html = document.documentElement;
  var body = document.body;
  if (!body) return;
  if (!html.classList.contains('fx-ready')) html.classList.add('fx-ready');

  // 1) Marcar .fx-in a los .fx-hide que ya están visibles ANTES de activar el
  //    ocultamiento: así la primera pinta no muestra contenido invisible (sin flash).
  var els = document.querySelectorAll('.fx-hide:not(.fx-in)');
  ...
  // 2) Activar el gating CSS (ocultar/revelar) sólo después de haber marcado lo visible.
  requestAnimationFrame(function () {
    html.classList.add('fx-activated');
  });

  // Re-init para contenidos cargados vía AJAX (reloadPosts etc.)
  window.reinitPageFx = function () {
    var nx = document.querySelectorAll('.fx-hide:not(.fx-in)');
    if (io) {
      nx.forEach(function (el) {
        var r = el.getBoundingClientRect();
        if (r.top <= viewH) el.classList.add('fx-in');
        else io.observe(el);
      });
    } else {
      nx.forEach(function (el) { el.classList.add('fx-in'); });
    }
  };

  // 3) bfcache (back/forward): nada quedado oculto
  window.addEventListener('pageshow', function (ev) {
    if (ev.persisted) {
      html.classList.add('fx-activated');
      document.querySelectorAll('.fx-hide').forEach(function (el) { el.classList.add('fx-in'); });
    }
  });
}
...
function reloadPosts(response) {
  if (response.success) {
    ...
    // Si hay tarjetas/items nuevos en la respuesta AJAX, re-inicializamos el observer
    if (typeof window.reinitPageFx === 'function') window.reinitPageFx();
  } else {
